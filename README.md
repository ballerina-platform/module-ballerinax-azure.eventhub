...
systems for processing, storage and analysis.

Azure Event Hubs [Ballerina](https://ballerina.io/) Connector is used to connect with the Azure Event Hubs to ingest millions of events per 
...
Execute the commands below to build from the source after installing the relevant Ballerina version. ( Run the following commands after setting the `eventhub` directory as the working directory )
...
2. To build the package without running the tests:
```shell script
